...
const getImageByPage = async (start, total, word) => {
  let allImages = [];

  while (start < total) {
    const size = Math.min(60, total - start);
    const res = await request(`https://image.baidu.com/search/acjson?tn=resultjson_com&ipn=rj&fp=result&queryWord=${encodeURI(word)}&ie=utf-8&oe=utf-8&word=${encodeURI(word)}&pn=${start}&gsm=${(start).toString(16)}&rn=${size}&${Date.now()}=`, headers_json);
...
const runImg = (keyword, counts, assetDir) => {
  // 调用common.js request, 传入headers_default
  request(
    `http://image.baidu.com/search/index?tn=baiduimage&ie=utf-8&word=${encodeURI(
      keyword
    ), headers_defalult}`
  ).then(async (res) => {
